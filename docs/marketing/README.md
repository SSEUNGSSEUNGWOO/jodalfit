...
- **자동 키워드** (`keywords.py`): 수동 목록 위에 실제 유입 검색어를 매일 자동으로 올린다. 네이버 최근 7일 누적 상위 검색어에서 클릭 2회 이상이거나, 일별 상위 검색어에 최근 14일 중 2일 이상 나온 것. 최대 40개. 조건에서 벗어나면 다음 계산에서 빠진다. 상태 파일 없이 매번 스냅샷에서 다시 계산하고, 성적표 6번 칸에 `*`와 "오늘 자동 추가·제외"로 표시한다. 수동 목록은 절대 안 빠진다.
